...
class AssistiveTeleoperation:
    def __init__(self):
        ...
        self.final_score = 0
        self.second_goal_pos = np.array([0.912, -0.189, 0.26])
        self.third_goal_pos = np.array([0.898, -0.029, 0.26])
        self.goal_pos = [self.second_goal_pos, self.third_goal_pos]
        self.intermediate_position = np.array([0.492, 0.25, 0.10])
        self.obj_pos = []
    ...
    def centroid_callback(self, centroid_msg):
        if MODE == 1 and self.human_input:
            first_obj_pos = np.array([0.814, 0.417, 0.07])
            first_obj_pose = np.array([0, 0, 0])

            self.obj_pos = [first_obj_pos]
            self.list_goal_poses = [first_obj_pose]
    ...
    def main(self):
        ...
        try:
            ...
            UserStudyExperiment.record_result("Shelving", MODE, task_duration=total_time, input_magnitude=self.final_score)
