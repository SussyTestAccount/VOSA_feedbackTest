...
class AssistiveTeleoperation:
    def __init__(self):
        ...
        print("Launching with Mode: ", MODE)
    ...
    def centroid_callback(self, centroid_msg):
        if MODE == 1:
            first_obj_pos = np.array([0.613, 0.482, 0.071])
            second_obj_pos = np.array([0.613, 0.258, 0.116])
    ...
    def compute_confidence(self, ur, predicted_goal_index, confidence_min=0.0, confidence_max=0.8):
...
if __name__ == "__main__":
    rospy.init_node('deceptive_grasping')
